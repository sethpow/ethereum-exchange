...
contract('Exchange', ( [ deployer, feeAccount, user1, user2 ] ) => {
    ...
    describe('making orders', async () => {
        ...
        it('tracks the newly created order', async () => {
            ...
            // examine new order that was created
                // fetch order from mapping
            const order = await exchange.orders('1')
                //check if details/attributes are correct
        ...
        describe('filling orders', async () => {
            ...
            describe('success', async () => {
                ...
                it('executes the trade & charges fees', async () => {
                    ...
                    balance.toString().should.equal(tokens(0.98).toString(), 'user2 tokens deducted with fee applied')
                    const feeAccount = await exchange.feeAccount()
                    balance = await exchange.balanceOf(token.address, feeAccount)
                    balance.toString().should.equal(tokens(0.02).toString(), 'feeAccount received fee')
                })
        ...
        describe('cancelling orders', async () => {
            ...
            describe('success', async () => {
                ...
                it('emits a "Cancel" event', async () => {
                    ...
                    event.user.should.equal(user1, 'user is correct')
                    event.tokenGet.should.equal(token.address, 'tokenGet is correct')
                    event.amountGet.toString().should.equal(tokens(1).toString(), 'amountGive is correct')
